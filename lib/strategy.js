 ...
 * body parameter, or `access_token` query parameter. These can de disabled
 * individually in the options parameter using tokenFrom.header, tokenFrom.body
 * and tokenFrom.query. Defaults to all.
 *
...
function Strategy(options, verify) {
  ...
  this._tokenFrom = _.defaults(options.tokenFrom || {}, {
    header: true,
    query: true,
    body: true
  });
...
Strategy.prototype.authenticate = function(req) {
  ...
  if (this._tokenFrom.header && req.headers && req.headers.authorization) {
  ...
  /* Don't complain about access_token */
  /*jshint camelcase: false */
  if (this._tokenFrom.body && req.body && req.body.access_token) {
    // Multiple tokens sent. Bad request.
    if (token) { return this.fail(400); }
    token = req.body.access_token;
  }

  if (this._tokenFrom.query && req.query && req.query.access_token) {
    // Multiple tokens sent. Bad request.
    if (token) { return this.fail(400); }
